...
| vtsingle_service_name                  | Name of the installed system service                                                                                                | `victoriatraces`                                                                                                                                                                                 |
...
| vtsingle_bin_dir                       | Location for binary file.                                                                                                           | `/usr/local/bin`                                                                                                                                                                                |
...
| vtsingle_service_envflag_data_file     | Role-managed env file holding the entries above. Rewritten on every run. | see [defaults.yml](./defaults/main.yml) |
| vtsingle_service_envflag_file          | User-managed env file, read after the role-managed one so its keys win. | see [defaults.yml](./defaults/main.yml)                                                                                                                                                          |
...
The role renders these entries into `vtsingle_service_envflag_data_file` (default `/etc/default/{{ vtsingle_service_name }}.env`, rewritten on every run) and lists it in the unit ahead of `vtsingle_service_envflag_file`, which the role creates empty and never rewrites. systemd reads `EnvironmentFile=` entries in listed order with the later one winning, so a key you set in `vtsingle_service_envflag_file` overrides the same key coming from `vtsingle_service_envflag_data`. Both files are created at mode `0600` owned by `root:root`; put secrets in `vtsingle_service_envflag_file`, the one the role never touches again.
