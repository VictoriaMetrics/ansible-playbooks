...
| vmsingle_service_name                       | Name of the installed system service                                                                                                | `victoriametrics`                                                                                                                             |
...
| vmsingle_bin_dir                            | Location for binary files.                                                                                                          | `/usr/local/bin`                                                                                                                              |
...
| vmsingle_service_envflag_data_file          | Role-managed env file holding the entries above. Rewritten on every run. | see [defaults.yml](./defaults/main.yml) |
| vmsingle_service_envflag_file               | User-managed env file, read after the role-managed one so its keys win. | see [defaults.yml](./defaults/main.yml)                                                                                                       |
...
The role renders these entries into `vmsingle_service_envflag_data_file` (default `/etc/default/{{ vmsingle_service_name }}.env`, rewritten on every run) and lists it in the unit ahead of `vmsingle_service_envflag_file`, which the role creates empty and never rewrites. systemd reads `EnvironmentFile=` entries in listed order with the later one winning, so a key you set in `vmsingle_service_envflag_file` overrides the same key coming from `vmsingle_service_envflag_data`. Both files are created at mode `0600` owned by `root:root`; put secrets in `vmsingle_service_envflag_file`, the one the role never touches again.

Each entry must be a single-line `KEY=value` string whose key matches `[A-Za-z_][A-Za-z0-9_]*` and whose value contains no `'`. The role asserts this and fails the play, reporting only the 1-based positions of the offending entries because values can hold secrets. Entries are written as `KEY='value'`, and systemd recognizes no escape sequences inside single quotes, so a `'` in a value would terminate it early - while `%` needs no doubling and `\` reaches the binary as written, which is what regex-valued flags need. An invalid key would be dropped by systemd silently (it logs `Ignoring invalid environment assignment` and starts the service anyway), which is why the assert exists.
