...
| vmsingle_version                            | Version to install                                                                                                                  | `v1.151.0`                                                                                                                                    |
...
The role rewrites `vmsingle_service_envflag_data_file` (default `/etc/default/{{ vmsingle_service_name }}.env`) on every run and lists it in the unit ahead of `vmsingle_service_envflag_file`, which the role creates empty and never rewrites, so a key you set there overrides the same key coming from `vmsingle_service_envflag_data`. Put secrets in `vmsingle_service_envflag_file`. The two paths must differ, and the role asserts it. Both files are kept at mode `0600` owned by `root:root`.
