...
| vlsingle_service_name                  | Name of the installed system service                                                                                              | `victorialogs`                                                                                                                                                                                  |
...
| vlsingle_bin_dir                       | Location for binary file.                                                                                                           | `/usr/local/bin`                                                                                                                                                                                |
...
The role renders these entries into `vlsingle_service_envflag_data_file` (default `/etc/default/{{ vlsingle_service_name }}.env`, rewritten on every run) and lists it in the unit ahead of `vlsingle_service_envflag_file`, which the role creates empty and never rewrites. systemd reads `EnvironmentFile=` entries in listed order with the later one winning, so a key you set in `vlsingle_service_envflag_file` overrides the same key coming from `vlsingle_service_envflag_data`. Both files are created at mode `0600` owned by `root:root`; put secrets in `vlsingle_service_envflag_file`, the one the role never touches again.
